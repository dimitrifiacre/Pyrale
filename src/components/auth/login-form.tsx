  ...
  const schema = useMemo(
    () =>
      z.object({
        ...
        password: z.string().min(1, tc("validation.required")),
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (!result || result.error) {
        const errorKey = result?.code ?? result?.error ?? "server_error"
        const knownErrors: Record<string, string> = {
          invalid_credentials: t("errors.invalid_credentials"),
          account_banned: t("errors.invalid_credentials"),
        }
        toast.error(tc("form.error.title"), {
          description: knownErrors[errorKey] ?? t("errors.server_error"),
        })
        return
      }
      ...
      toast.error(tc("form.error.title"), { description: t("errors.server_error") })
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6" noValidate>
      ...
      <div className="flex items-center justify-between">
        <Field orientation="horizontal">
          ...
          />
          <FieldContent>
            <FieldLabel htmlFor="rememberMe">{t("remember_me")}</FieldLabel>
          </FieldContent>
        </Field>
        <Link href="/forgot-password" className="text-nowrap text-sm text-primary hover:underline">
          {t("forgot_password")}
        </Link>
      </div>
